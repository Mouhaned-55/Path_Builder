...
class _PathEditorState extends State<PathEditor> {
  ...
  Future<void> _importDrawing() async {
    ...
    if (result != null && result.files.isNotEmpty) {
      setState(() {
        backgroundImage =
            File(result.files.first.path!); // Set background image
      });
    }
  }
  ...
  Future<void> _exportDrawing() async {
    ...
    for (var pathData in paths) {
      if (pathData.points.isNotEmpty) {
        svgPath.write(
            '<path d="M${pathData.points[0].dx},${pathData.points[0].dy} ');
        ...
        for (int i = 0; i < pathData.points.length - 1; i++) {
          if (pathData.isControlPointModified[i]) {
            ...
            svgPath.write(
                'L${pathData.points[i + 1].dx},${pathData.points[i + 1].dy} ');
          }
        }
  ...
  void _selectPointOrAddNew(Offset position) {
    ...
    for (int i = 0; i < currentPath.points.length; i++) {
      if (_isWithinProximity(
          currentPath.points[i], position, proximityThreshold)) {
        _selectPoint(i);
        return;
      }
      if (_isWithinProximity(
          currentPath.controlPointsOut[i], position, proximityThreshold)) {
        _selectControlPoint(i, isOutward: true);
        return;
      }
      if (_isWithinProximity(
          currentPath.controlPointsIn[i], position, proximityThreshold)) {
  ...
  void _movePointAndControlPoints(int index, Offset delta) {
    setState(() {
      ...
      currentPath.controlPointsIn[index] =
          currentPath.controlPointsIn[index]! + delta;
      currentPath.controlPointsOut[index] =
          currentPath.controlPointsOut[index]! + delta;
    });
  }
  ...
  void _removeSelectedPoint() {
    if (paths[currentPathIndex].selectedPointIndex != null) {
      setState(() {
        ...
        currentPath.isControlPointModified
            .removeAt(currentPath.selectedPointIndex!);
